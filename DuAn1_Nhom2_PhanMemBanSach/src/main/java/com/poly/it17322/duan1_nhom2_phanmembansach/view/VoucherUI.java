...
public class VoucherUI extends javax.swing.JPanel {
    ...
    public List<Voucher> locHD() {
        List<Voucher> list;
        int index = cboTimTT.getSelectedIndex();
        if (index == 0) {
            list = vcService.getList();
        } else if (index == 1) {
            list = vcService.locTT(true);
    ...
    private void initComponents() {
        ...
        cboTimTT.addItemListener(new java.awt.event.ItemListener() {
            public void itemStateChanged(java.awt.event.ItemEvent evt) {
                cboTimTTItemStateChanged(evt);
            }
        });
    ...
    private void cboTimTTItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_cboTimTTItemStateChanged
        // TODO add your handling code here:
        if (cboTimTT.getSelectedIndex() == 0) {
            loadData(vcService.getList());
        } else if (cboTimTT.getSelectedIndex() == 1) {
            List<Voucher> list = vcService.locTT(true);
            loadData(list);
        } else {
            List<Voucher> list = vcService.locTT(false);
            loadData(list);
        }
    }//GEN-LAST:event_cboTimTTItemStateChanged

    public void loadCBOHD() {
        combo = (DefaultComboBoxModel) cboTimTT.getModel();
        combo.removeAllElements();
        List<String> listTT = new ArrayList<>();
        listTT.add("Tất cả");
        listTT.add("Hoạt động");
        listTT.add("Hết hạn");
